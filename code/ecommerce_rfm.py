...
parser = argparse.ArgumentParser(description="Run the UK e-commerce and RFM pipeline")
parser.add_argument("--data", type=Path, default=Path("data/data.csv"),
                    help="Path to the Online Retail CSV")
parser.add_argument("--output", type=Path, default=Path("outputs/ecommerce"),
                    help="Directory for model-ready tables")
parser.add_argument("--encoding", default="latin1", help="CSV encoding; UCI source uses latin1")
...
# 1. 读取、类型转换与质量画像 -------------------------------------------------
raw = pd.read_csv(DATA_PATH, encoding=args.encoding)
...
rfm.reset_index().to_csv(OUTPUT_DIR / "customer_rfm.csv", index=False)
